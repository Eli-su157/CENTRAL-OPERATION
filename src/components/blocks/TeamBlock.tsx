...
export function TeamBlock({ realTeam }: Props) {
  return (
    <div className="relative bg-[#161616] border border-white/[0.06] rounded-xl p-5 shadow-card overflow-hidden">
      ...
      {!realTeam ? (
        <EmptyState
          title="Nenhum dado de equipe ainda"
          description="Adicione membros e tarefas para ver o resumo aqui."
          iconVariant="neutral"
          className="py-4"
        />
      ) : (
        <>
          <div className="grid grid-cols-3 gap-3 mb-4">
            <StatCell
              label="Atrasadas"
              value={formatNumber(realTeam.tarefas_atrasadas.reduce((s, x) => s + x.quantidade, 0))}
              valueClass={realTeam.tarefas_atrasadas.reduce((s, x) => s + x.quantidade, 0) > 0 ? 'text-red-400' : 'text-emerald-400'}
              highlight={realTeam.tarefas_atrasadas.reduce((s, x) => s + x.quantidade, 0) > 0}
            />
            <StatCell
              label="Pendentes"
              value={formatNumber(realTeam.tarefas_pendentes_total)}
              valueClass={realTeam.tarefas_pendentes_total > 5 ? 'text-amber-400' : 'text-white'}
            />
            <StatCell label="Membros" value={formatNumber(realTeam.membros_ativos)} />
          </div>

          {realTeam.tarefas_atrasadas.length > 0 && (
            <div className="pt-4 border-t border-white/[0.05]">
              <p className="text-[10px] text-zinc-600 uppercase tracking-[0.1em] font-semibold mb-3">Atrasos por setor</p>
              <div className="flex flex-col gap-1.5">
                {realTeam.tarefas_atrasadas.map(item => (
          ...
          {realTeam.tarefas_atrasadas.reduce((s, x) => s + x.quantidade, 0) === 0 && realTeam.tarefas_pendentes_total === 0 && (
