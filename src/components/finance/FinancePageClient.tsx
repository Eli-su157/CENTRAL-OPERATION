...
export function FinancePageClient({ entries, categories, dashboards, members }: Props) {
  ...
  return (
    <div>
      ...
      {/* Lucro líquido hero */}
      <div className={`relative rounded-2xl border p-6 mb-6 overflow-hidden ${
        dre.lucro_liquido >= 0
          ? 'bg-emerald-500/5 border-emerald-500/15'
          : 'bg-red-500/5 border-red-500/15'
      }`}>
        <div className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent ${
          dre.lucro_liquido >= 0 ? 'via-emerald-500/40' : 'via-red-500/40'
        } to-transparent`} />
        <p className="text-[10px] text-zinc-500 uppercase tracking-[0.1em] font-semibold mb-2">{label} · Lucro líquido</p>
        <p className={`text-[2.5rem] font-bold tabular-nums leading-none ${dre.lucro_liquido >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
          {formatCurrency(dre.lucro_liquido)}
        </p>
        <div className="flex gap-6 mt-4">
          <span className="text-sm text-zinc-500">
            Receita <span className="text-emerald-400 font-semibold ml-1">{formatCurrency(dre.receita_bruta)}</span>
          </span>
          <span className="text-sm text-zinc-500">
            Custos <span className="text-red-400 font-semibold ml-1">
